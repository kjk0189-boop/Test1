...
- 크루원: **카메라로 실제 QR 코드를 스캔**해서 출/퇴근, 내 근무 기록(월별 조회)
- 매니저: 출근 현황, 근태 수정(+수정 이력 로그), 크루원 등록/수정/삭제, 급여 관리(연장·야간·주휴수당 자동 계산), 매장 QR 출력(실제 스캔 가능한 이미지)
- 관리자: 통합 대시보드, 지점 운영(지점 선택 후 매니저 화면 대행 — QR 포함), 지점 관리(신규 지점 추가), 매니저 관리(등록/수정/삭제), 전 지점 급여, 근태 수정 이력(전 지점)
- 계정 설정(비밀번호 변경) 공통 기능

## 아직 옮기지 않은 기능 (다음 단계)

근로계약서(서명), 급여명세서 발급·저장 이력, **GPS 위치 기반 출퇴근 위치 검증**(카메라 QR은 이번에 추가했지만, "매장 반경 안에서만 인정" 같은 위치 검증은 아직 없어요) — 이 부분들은 다음 단계로 미뤄뒀어요.
