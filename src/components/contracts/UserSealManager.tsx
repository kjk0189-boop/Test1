...
export default function UserSealManager({
  userId,
  sealImage,
  onUpdated,
}: {
  ...
  return (
    <div className="rounded-xl border bg-white p-4 mb-6 flex items-center justify-between flex-wrap gap-3" style={{ borderColor: "#DDE1D8" }}>
      <div className="flex items-center gap-3">
        ...
        <div>
          <div className="text-sm font-semibold" style={{ color: "#1B2420" }}>회사(사업주) 인감</div>
          <div className="text-xs" style={{ color: "#8A9088" }}>
            {sealImage ? "등록됨 · 크루/매니저 계약서 서명 시 모두 이 인감이 사용돼요" : "등록하면 모든 근로계약서 서명 시 직접 그리지 않고 이 인감을 사용할 수 있어요"}
          </div>
